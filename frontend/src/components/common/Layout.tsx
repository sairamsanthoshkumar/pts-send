...
import { LayoutDashboard, FlaskConical, LogOut, Dna } from 'lucide-react'
    ...
    <div className="flex h-screen overflow-hidden bg-slate-950">
      <aside className="w-60 flex-shrink-0 bg-slate-900 border-r border-slate-800 flex flex-col">
        <div className="px-5 py-5 flex items-center gap-3 border-b border-slate-800">
          ...
          <div><p className="font-semibold text-sm text-white">PtsSEND</p><p className="text-xs text-slate-500">SEND Platform</p></div>
        </div>
        ...
        <div className="px-3 py-4 border-t border-slate-800">
          ...
          <button onClick={() => { logout(); navigate('/login') }} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-slate-400 hover:text-slate-200 hover:bg-slate-800 transition-colors">
